...
# run-pi-pane.sh
#
# Run `pi` as a headless subagent (--mode json) inside a visible herdr
# tab so the user can watch it work, while the full JSON event stream is
...
# to completion (not inside herdr, closed tab); pi's own exit code is
...
if [ -z "${HERDR_ENV:-}" ]; then
  echo "run-pi-pane.sh: not inside a herdr session, cannot open a tab" >&2
  echo "Run the agent from inside herdr, then try again." >&2
  exit 1
fi

# Sanitize into a readable herdr tab label.
TAB_LABEL="$(printf '%s' "${TASK_NAME:-pi-task}" | tr '[:upper:]' '[:lower:]' | sed -E 's/[^a-z0-9]+/-/g; s/^-+//; s/-+$//')"
TAB_LABEL="${TAB_LABEL:-pi-task}"

# Resolve tooling absolutely in the agent's shell (which has the right
# PATH, e.g. fnm's pi shim). The herdr pane inherits the server's
...
# into the `herdr pane run` command line — the pipeline itself contains
# single quotes, so any inline-quoting scheme is a foot-gun. The prompt
# text is read via $(cat ...) inside the pane, so arbitrary prompt content
# never touches that command line. pi's exit code is taken from the head
# of the pipe (PIPESTATUS) and recorded before the sentinel is touched,
# so the waiter below always has a status to report. herdr returns the
# pane to its zsh prompt when the script finishes, so the transcript
# stays on screen with no `exec` needed — the user closes it themselves.
cat > "$PANE_SCRIPT" <<EOF
#!/usr/bin/env bash
...
printf '\n\n---- pi finished (exit %s) ---- (Ctrl-D to close this tab)\n' "\$st"
touch '$SENTINEL'
EOF
...
# current repo so pi's relative paths and context-file discovery resolve
...
# Block until pi finishes (sentinel appears). The missing-pane guard means
...
if [ "$aborted" = 1 ]; then
  echo "STATUS=aborted"
  echo "run-pi-pane.sh: tab closed before pi finished; run is incomplete" >&2
  exit 1
fi
