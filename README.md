...
`ctrl+s` prefix, prefix-free `ctrl+h/j/k/l` pane focus, `|` and `\` for a
side-by-side split, `prefix+d` detach, `prefix+,` rename, `prefix+&` close
tab, and `prefix+r` reload. Much of `.tmux.conf` already matches herdr's
defaults; the handful of bindings that cannot be ported at all are listed in
the config's own comments. Run
